...
int uncompress_pipeline(
    ...
    IO::PairWriter pair_writer(std::move(out_pairs), order / compress);
    IO::PairReader pair_reader(in_pairs, order / compress);
    for(IO::PairType pair; pair_reader >> pair;) {
        ...
        match_pairs(order, new_compress, paf_constant, ina, inb, [&](IO::PairType pair) {
            pair_writer << pair;
        });
    }
    return 0;
