...
#include "cufftdx/operators/precision.hpp" // is_supported_fp_type
#include "cufftdx/types.hpp"
...
#define CUFFTDX_DETAIL_DEFINE_MAKE_COMPLEX_TYPE(scalar_type, v_type)                              \
    template<>                                                                                    \
    struct make_complex_type<scalar_type> {                                                       \
        using cufftdx_type                             = ::cufftdx::detail::complex<scalar_type>; \
        static constexpr value_type cufftdx_value_type = v_type;                                  \
    };

        CUFFTDX_DETAIL_DEFINE_MAKE_COMPLEX_TYPE(float, value_type::c_32f)
        CUFFTDX_DETAIL_DEFINE_MAKE_COMPLEX_TYPE(double, value_type::c_64f)

#if (!defined(__CUDA_ARCH__) || __CUDA_ARCH__ >= 530)
        template<>
        struct make_complex_type<__half> {
            using cufftdx_type                             = ::cufftdx::detail::complex<__half2>;
            static constexpr value_type cufftdx_value_type = value_type::c_16f2;
        };
